// Path: Rive_Animation/Home_schools.tsx
// Code file ID: frGiDQi
// Used on: Programs/Parents pages (desktop) — same .riv as Eco_system.tsx but SCROLL_PX_FOR_FULL_PROGRESS = 14000
...
// SCROLL_PX_FOR_FULL_PROGRESS = 14000 (vs 10000 for home) | PROGRESS_MAX = 1000 | START_DELAY_PX = 650
// NOTE: Same component name "HomeDesktop" as Eco_system.tsx — different scroll sensitivity
...
const SCROLL_PX_FOR_FULL_PROGRESS = 14000
...
export default function HomeDesktop() {
    ...
    useMotionValueEvent(scrollY, "change", (latestY) => {
        ...
        if (next !== progressRef.current) { progressRef.current = next; setProgress(next) }
    })
    ...
    useEffect(() => {
        ...
        const onTouchStart = (e: TouchEvent) => { if (!e.touches?.length) return; const { x, y } = rectToCanvasXY(e.touches[0]); (riveInstance as any).pointerDown?.(x, y) }
        const onTouchMove = (e: TouchEvent) => { if (!e.touches?.length) return; const { x, y } = rectToCanvasXY(e.touches[0]); (riveInstance as any).pointerMove?.(x, y) }
        const onTouchEnd = (e: TouchEvent) => { const t = e.changedTouches?.[0]; if (!t) return; const { x, y } = rectToCanvasXY(t); (riveInstance as any).pointerUp?.(x, y) }
    ...
    return (
        <section ref={sectionRef} style={{ textAlign: "center", width: "100%", height: "100vh", position: "relative", touchAction: "pan-y" }}>
            ...
            {!isInView && (
                <img src={IMAGE_PLACEHOLDER} alt="Placeholder" style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }} />
            )}
        </section>
    )
}
