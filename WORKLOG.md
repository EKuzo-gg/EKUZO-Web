...
- **`lib/authors.ts` (new) — the one place author facts live.** Before this, every author fact was hand-copied per page: Muhammad's role line existed in 3 files, his image path in 4, and the byline block was duplicated into all 10 posts with three different date formats between them (`Published May 24, 2026`, `Updated May 14, 2026`, bare `Jan 14, 2026`). Registry holds slug, name, kind, role, avatar, headshot (+ intrinsic dims), description, bio, pull-quote, `personId`, `hasPage`, `footerBanner`, `noindex`. `buildAuthorMetadata()` builds each author page's `metadata` from it.
- **`components/blog/AuthorByline.tsx` (new).** Mobile-first per Jamie's call, since the blog is read mostly on phones: horizontal row under `lg` (44px circle left, name and meta beside it, two lines), stacked sticky rail at `lg` and up (64px circle above the meta). The old block collapsed to a tall stack on small screens and pushed the first paragraph down. Guest authors render name and date only: no avatar, no link, no role line. Deliberately nothing in the avatar slot rather than a monogram, so the two classes read as visibly different.
- **`components/blog/BlogPostBody.tsx` (new).** Wraps the byline rail + `BlogContent`. Exists so read time is computed from the post body at render time (`lib/readingTime.ts`, 238 wpm per Brysbaert 2019) instead of hand-maintained per post. Suppressed above 12 minutes, where the label stops reading as permission and starts reading as a warning. Nothing on the site is over 11 today.
- **`components/blog/AuthorPage.tsx` (new).** Collapses the three near-identical author pages, which had drifted: Karlin's had a pull-quote and no role line, Jamie's and Muhammad's had a role line and no pull-quote; Karlin's post grid was always on while the other two hid when empty. All three now get a role line, an optional pull-quote, and a conditional grid. Each route file keeps its own `POSTS` array and `metadata` export. Post lists deliberately did NOT move to a shared index: the commented-out cards on Muhammad's page are one of the six leak-guard layers, and consolidating them would move the guard.
- **Muhammad's FooterBanner exception is now data, not a hardcode** (`footerBanner: false`). Same reasoning as before: the only route onto his page is a Kassi byline, and both posts dropped the banner as non-promotional.
- **Karlin's assets.** `public/images/coach-karlin-faith.jpg` is 6349x4312 / 10.2 MB and was being CSS-cropped on every load of his author page. Two derivatives in `public/images/authors/`, same crop intent: `coach-karlin-faith-1400.jpg` (1400x951, 159 KB) for the page hero, `karlin-oei-avatar.jpg` (256x256, 14 KB) for the byline circle. That is ~10.5 MB off every load of his author page. Avatars for Jamie and Muhammad derived the same way from their existing headshots. The action shot stays: it reads as "competitive gaming founder," which is a stronger signal than a polite headshot.
- **`lib/schema.ts`:** Karlin's Person `image` repointed at the 1400px derivative. Nothing else changed. The 7/28 audit claimed Karlin resolved as a non-Person entity and had no `sameAs`; both were wrong. `coachKarlinSchema` is already `"@type": "Person"` (only the `@id` fragment says "coach", and it is load-bearing for `founder` / `instructor` references), and he already carries a LinkedIn `sameAs`. All three staff authors already had `knowsAbout`. No schema churn was needed.
- **Leak guard re-verified on all six layers** after the refactor, on all three dark pages: `robots: index:false` (Muhammad's now via `noindex: true` in the registry), the three `X-Robots-Tag` rules in `next.config.mjs`, absence from `sitemap.ts`, commented-out cards in `app/blog/page.tsx`, the AI-crawler block in `public/robots.txt`, and the commented-out cards in Muhammad's `POSTS`. His TO PUBLISH checklist was rewritten to point at the registry, since steps (a) and (e) used to name fields that have moved.

- **Draft image placeholders now read like a draft.** The three `WireframePlaceholder` blocks across the two Kassi posts were rendering full production briefs on the page: pixel dimensions, output filenames, internal shorthand, and Kassi's own name. She reads these drafts, and a spec-laden placeholder makes an unfinished page look much further along than it is, which is the opposite of what a placeholder is for. Rendered copy is plain intent now ("an opening image goes here", one sentence on what it should do, no faces and nothing identifying). The briefs moved verbatim into JSX comments directly above each placeholder, so nothing was lost for production.

**Notes for Aaron:** `/programs/ekuzo-camps` and `/programs/ekuzo101` coach cards were repointed at `authors/coach-karlin-faith-1400.jpg` as well, same framing and `objectPosition` untouched. Nothing on the site references the 10.2 MB original any more, so `public/images/coach-karlin-faith.jpg` can be deleted. Verification recipe now that `next build` times out on the device VM: a `tsconfig.json`-extending config with a narrow `include` typechecks the blog surface in ~1.5s.

**Next:** the Kassi send (Muhammad sends, Karlin cc). On her blessing, run all three TO PUBLISH checklists together.
