...
**Notes for Aaron:** `/programs/ekuzo-camps` and `/programs/ekuzo101` still serve the 10.2 MB `coach-karlin-faith.jpg` in their coach cards. Repointing both at `authors/coach-karlin-faith-1400.jpg` is a two-line change with the same crop intent, left alone because it is your lane. Verification recipe now that `next build` times out on the device VM: a `tsconfig.json`-extending config with a narrow `include` typechecks the blog surface in ~1.5s.
