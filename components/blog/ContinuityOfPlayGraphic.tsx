 ...
 *
 * TWO RENDERS, ONE GRAPHIC:
 *   desktop (md and up) — the approved 2000x700 artwork, unchanged geometry,
 *     with the two embedded font faces dropped. It now draws with the site's
 *     own Tungsten and Inter via var(--font-display) / var(--font-body), which
 *     is why this file is 10KB and not the 298KB the standalone SVG weighs.
 *   mobile (below md) — the same three panels stacked, with every piece of
 *     TYPE moved out of the SVG and into HTML. That is deliberate: HTML text
 *     wraps when a font resolves wider than expected, where SVG text would
 *     just overflow the viewBox. The SVGs below carry the node diagram only.
 *
...
export default function ContinuityOfPlayGraphic() {
  return (
    <figure className="my-10 bg-[#f0edea]">
      ...
      {/* ── Mobile: same panels stacked, type in HTML so it wraps ───────── */}
